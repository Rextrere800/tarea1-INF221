...
a partir de los archivos de medidas producidos por matrix_multiplication.cpp.

Formato esperado de cada archivo en ./data/measurements:
    nombre:    {algoritmo}_{n}_{tipo}_{dominio}_{muestra}_out.txt
    contenido: "{tiempo_microsegundos} {memoria_pico_bytes}"

Donde:
    algoritmo in {naive, strassen}
    n         in {16, 64, 256, 1024}      (2^4, 2^6, 2^8, 2^10)
    tipo      in {dispersa, diagonal, densa}
    dominio   in {D0, D10}
    muestra   in {a, b, c}
...
PATRON = re.compile(
    r"^(?P<algo>naive|strassen)_"
    r"(?P<n>\d+)_"
    r"(?P<tipo>dispersa|diagonal|densa)_"
    r"(?P<dominio>D0|D10)_"
    ...
    for tipo in TIPOS:
        for dominio in DOMINIOS:
            ...
            if hubo_datos:
                ...
                ax.set_yscale("log")
                ax.set_xlabel("n (dimension de la matriz cuadrada)")
                ax.set_ylabel("tiempo promedio (s)")
                ax.set_title(f"Tiempo de multiplicacion - {tipo} - dominio {dominio}")
                ax.legend()
